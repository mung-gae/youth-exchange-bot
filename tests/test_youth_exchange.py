...
class YouthExchangeServiceTest(unittest.TestCase):
    ...
    def test_tier_buyout_sale_cancel_lookup_next_round_and_end(self):
        ...
        event = self.svc.db.execute('SELECT status,current_round,selected_season,sale_start_at,sale_end_at FROM events WHERE event_id=?', (self.event_id,)).fetchone()
        self.assertEqual(event['status'], 'SETTING')
        self.assertEqual(event['current_round'], 2)
        self.assertIsNone(event['selected_season'])
        self.assertIsNone(event['sale_start_at'])
        self.assertIsNone(event['sale_end_at'])
        self.svc.start_buying(self.event_id)
        self.svc.purchase(self.event_id, 'u1', 1, 1)
        self.svc.close_buying(self.event_id)
