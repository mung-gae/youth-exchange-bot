...
- 다음 라운드는 `SETTING` 준비 상태로 돌아가므로 공개된 아이템 확인/추가 공개 → 구매 시작 → 구매 종료 → 매입금 설정 순서로 매 라운드를 반복할 수 있습니다.
...
`/계절투자 이벤트관리 다음라운드`를 실행하면 라운드 번호가 증가하고 상태가 `SETTING`으로 변경됩니다. 이전 라운드에서 판매하지 않은 아이템과 자금은 유지되며, 운영진은 공개 상태를 확인하거나 아이템을 추가 공개한 뒤 다시 구매를 시작할 수 있습니다.
