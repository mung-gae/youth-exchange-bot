...
관리자는 참가자의 구매, 판매 신청, 판매 변경, 판매 취소, 정산, 기본금 및 관리자 자금 지급을 관리자 전용 채널에서 확인할 수 있습니다.
...
거래 로그에는 참가자의 구매 아이템, 수량, 잔액, 판매 신청량, 정산 요약이 표시됩니다. 반드시 일반 유저가 볼 수 없는 관리자 전용 채널로 설정하세요.
...
거래 활동은 `trade_activity_logs` 테이블에 먼저 저장되고, Discord 전송 성공 시 `delivered_at`과 `discord_message_id`가 기록됩니다. 전송 실패 시 거래 자체는 롤백하지 않으며, 봇 시작 또는 다음 전송 시 최근 미전송 로그를 ID 기준으로 재시도합니다.
