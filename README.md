...
8. 디스호스트 콘솔에서 `npm ci --omit=dev`와 `npm start`를 실행합니다.

### 사용자가 직접 빌드해 업로드하는 방법
디스호스트에 올릴 파일은 사용자가 로컬 PC에서 직접 `npm run build`를 실행한 뒤 준비할 수도 있습니다.
...
- `youth-exchange-bot-dishost.zip` 파일은 Git에 커밋하지 않고 GitHub Actions 아티팩트로 제공합니다.
...
## 업로드 ZIP
GitHub Actions의 `Build Dishost Package` 워크플로가 `youth-exchange-bot-dishost.zip`을 생성하고 `청춘거래소-디스호스트` 아티팩트로 업로드합니다. ZIP은 Git에 커밋하지 않습니다.
